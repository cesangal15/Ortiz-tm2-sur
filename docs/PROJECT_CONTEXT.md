...
**Roles:** 5 capataces (actividades + equipos), 4 chequeadoras (viajes por PK destino, por origen; desglose por placa), 1 encargado (reconcilia y aprueba), admin (todo, vía menu.html).
...
- Chequeadora genera excavación (por origen: Masivo2/Masivo1/Diviso/Complementario/Otro) + terraplén solo si destino=Terraplén (Puente/ODL/Botadero no). El total de viajes por línea se **calcula** del desglose por placa que pega la chequeadora (bloque estilo WhatsApp; placa 3+3, viajes tras el guion); cada placa va a la hoja VOLQUETAS. El volumen oficial no cambia (D48 sobre D06).
...
**Entidades:** BANDEJA(22 cols, estado) · DATA(A–T maestro + 7 internas) · MAQUINARIA(18 cols, con produccion/unidad_prod; pendiente alinear a Captura_Diaria) · VOLQUETAS(10 cols: id_registro·timestamp·fecha·reporta·origen·destino·tipo_destino·uf·placa·viajes; 1 fila/placa, informativo, no va a DATA) · Catálogo 29 actividades (23 productivas + 6 de apoyo, ver 05_CATALOGO) · 20 máquinas. Formulario capataz incluye resumen en vivo (actividades, producción, equipos, validación pre-envío).
