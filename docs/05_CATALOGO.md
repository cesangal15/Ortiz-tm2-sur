...
**cesar**/venganza753 → menu (**D121:** el usuario del administrador se llama `cesar`, no `admin`; el **rol** sigue siendo `admin` y no cambia en ningún guard. La clave se guarda como hash de `usuario:clave`, así que renombrar la fila en la hoja `USUARIOS` obliga a volver a pegar la contraseña y correr `endurecerClaves()`) · encargado/enc1-2 → encargado · residente/Ortiz2026 → residente · jefe/Ortiz2026 → jefe · capataz1-5/uf1-2 → reporte-capataz · chequeadora1-3/cheq1-2 → reporte-chequeadora.
