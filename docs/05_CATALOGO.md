...
**Drenajes (D70, claves placeholder):** capataces NOMINALES por área (usuario propio → trazabilidad en `reporta`, como los de tierras; el **rol** `capataz_odt`/`capataz_odl` es lo que activa el modo del formulario, clave común `dren2026`): **ODT** = `mauricio`, `eduardo`, `enrique`; **ODL** = `jairo`. Todos → `reporte-drenajes.html`. Residentes `residente_odt` y `residente_odl` (clave `Ortiz2026`, roles `residente_odt`/`residente_odl`) → `residente-drenajes.html` (bandeja/envío/WhatsApp SOLO de su área). El admin entra a ambas pantallas desde `menu.html` con `?area=odt|odl`. La lista de "capataces esperados" del panel del residente (`CAPATACES_ESPERADOS_POR_AREA`) usa estos nombres.
