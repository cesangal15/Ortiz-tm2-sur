...
(async()=>{
  ...
  console.log('\n1 · parte.html?eq=VOL048 (390px): precarga y envío de 2 tramos');
  {
    ...
    ok('pantalla de confirmación con resumen', (await $(pg,'#okMsg').textContent()).includes('2 tramo(s)'));
  ...
  console.log('\n3 · parte.html sin eq: selector de respaldo · Día sin operación');
  {
    ...
    ok('pantalla «Escanea el QR» con la lista de equipos activos', (await $(pg,'#selEquipo option').count())===5);
  ...
  console.log('\n4 · revision-maquinaria.html (1440px, admin): pendientes, alertas, aprobar/editar/descartar, manual');
  {
    ...
    ok('carga las 4 pendientes del día', n===4, String(n));
    ...
    ok('el botón «Aprobar todo lo sin alertas» cuenta 3', /\(3\)/.test(await $(pg,'#btnAprobarTodo').textContent()));
    ...
    await $(pg,'.fila.dirty button:has-text("Aprobar")').click(); await pg.waitForFunction(()=>document.querySelectorAll('#pendientes .fila').length===3);
    ...
    await pg.waitForFunction(()=>document.querySelectorAll('#pendientes .fila').length===2);
    ...
    await $(pg,'#mGuardar').click(); await pg.waitForFunction(()=>document.querySelectorAll('#pendientes .fila').length===3);
    ...
    ok('Base: 3 aprobadas en el rango', filasBase===3, String(filasBase));
    ...
    ok('«Copiar para Excel»: 3 líneas de 43 columnas (B→AR)', lineas.length===3 && lineas.every(l=>l.split('\t').length===43), lineas.map(l=>l.split('\t').length).join(','));
  ...
  console.log('\n5 · Guards y menú');
  {
    ...
    ok('los 15 accesos anteriores siguen (17 en total)', await $(pg3,'a.tile').count()===17);
