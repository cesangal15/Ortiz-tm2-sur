...
| 5 · Investigación PDF | Cargas los PDFs de partes y corres el OCR | Búsqueda dirigida SOLO de las no-encontradas: pasada roja por franjas (número impreso arriba-derecha, 1–3 partes/página) + pasada gris de respaldo (AVENSA). Verde = exacto, naranja = 1 dígito. NUNCA auto-confirma: tú ves la página y decides (Confirmar / Es ASFALTO / No es). Además, el panel "Cobertura del OCR por página" muestra la vista inversa: páginas SIN lectura o cuyo número no coincide con nada reclamado, listas para revisarlas a mano; con ✏️ puedes corregir la lectura del OCR (si el número corregido es una faltante pasa a candidato; si es de una remisión ya conciliada, la página se descarta de la revisión). También salen en el resumen del Paso 7. Navegador manual de miniaturas siempre disponible. |
